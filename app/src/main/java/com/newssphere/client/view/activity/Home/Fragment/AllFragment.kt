...
class AllFragment : Fragment(), HomeCategoriesCommunicator {
    ...
    companion object{
        ...
        fun newInstance(deliveredCategory: String?= null): AllFragment{
    ...
    private fun setUpView(){
        currentPage = 1
        homeViewModel.getNewsCollection(category = null, page = currentPage!!)
        homeViewModel.isLoading.observe(this@AllFragment.requireActivity(), {
            if(it) categoriesHomeCommunicator.startLoading() else categoriesHomeCommunicator.stopLoading()
        })

        homeViewModel.isFail.observe(this@AllFragment.requireActivity(), {
            if(it){
                this@AllFragment.requireActivity().showPopUpDialog(
                    getString(R.string.tvPopUpDescription),
                    R.drawable.earth_cry,
                    object: PopUpDialogListener{
                        override fun onClickListener() {
                            this@AllFragment.requireActivity().closeOptionsMenu()
                        }
                    }
                )
            }
                    ...
                    itemNewsAdapter = ItemNewsAdapter(
                        ...
                        object: ItemNewsAdapter.ItemListener{
                            override fun onItemClicked(item: Article) {
                                this@AllFragment.requireActivity().startActivity(
                                    DetailActivity.newIntent(
                                        this@AllFragment.requireActivity(),
                                        deliveredCategory!!,
                                        item
                                    )
                                )
                                this@AllFragment.requireActivity().overridePendingTransition(R.anim.slide_in_right, R.anim.slide_out_left)
                            }
                        })

                    val myLayoutManager = LinearLayoutManager(this@AllFragment.requireActivity())
